...
def _add_structure_box(
    ...
    *,
    ...
    if not bounds:
        return
    key = (page_index, tuple(bounds), str(box_type), str(label), str(text))
    if key in seen:
        return
    seen.add(key)
    entry: Dict[str, Any] = {
...
def _collect_boxes_recursive(
    ...
    *,
    ...
    if isinstance(obj, dict):
        for key in ("bbox", "box", "bounds", "coordinate", "points", "polygon", "poly"):
            if key in obj:
                bounds = _bounds_from_box(obj.get(key))
                label = obj.get("label") or obj.get("type") or obj.get("name")
                text = obj.get("text") or obj.get("rec_text")
...
def _collect_structure_boxes(raw_result: Any) -> List[Dict[str, Any]]:
    ...
    for page_index, page in enumerate(pages):
        if not isinstance(page, dict):
            continue
        layout_boxes = (page.get("layout_det_res") or {}).get("boxes") or []
        for item in layout_boxes:
            ...
            _add_structure_box(
                boxes,
                seen,
                bounds=_bounds_from_box(item.get("coordinate") or item.get("bbox")),
                box_type="layout",
                label=item.get("label") or "layout",
                score=item.get("score"),
                page_index=page_index,
            )
        region_boxes = (page.get("region_det_res") or {}).get("boxes") or []
        ...
        ocr_summary = _summarize_ocr(page.get("overall_ocr_res") or {})
        for line in ocr_summary.get("lines") or []:
            if not isinstance(line, dict):
                continue
            _add_structure_box(
                boxes,
                seen,
                bounds=_bounds_from_box(line.get("bounds")),
                box_type="ocr",
                text=line.get("text"),
...
def _build_structure(raw_result: Any) -> Dict[str, Any]:
    return {
        "pages": raw_result,
        "summary": _summarize_structure(raw_result),
        "boxes": _collect_structure_boxes(raw_result),
    }
...
async def ocr_endpoint(
    ...
    if raw:
        ...
        payload["structure"] = _build_structure(raw_result)
    if annotate:
